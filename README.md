# Sistema de Pasajeros - Hotel Duerme Bien

Proyecto basado en el **Caso 6: Sistema de Pasajeros de Hotel** entregado por el profesor.

## Diseño de la página

La página funcional usa como referencia las mismas etapas visuales que están en el repositorio: **Sketch → Wireframe → Mockup → Prototipo**.

### Sketch

![Sketch del sistema](entrega-final/07-sketch.svg)

### Wireframe

![Wireframe del sistema](entrega-final/08-wireframe.svg)

### Mockup

![Mockup del sistema](entrega-final/09-mockup.svg)

## Evaluaciones completas visibles en la página

- [Evaluación 1 - IEEE 830, entrevista, factibilidad y trazabilidad](prototipo-web/evaluacion1.html)
- [Evaluación 2 - UML, flujo, base de datos, interfaz y Kanban](prototipo-web/evaluacion2.html)
- [Checklist exacto de la pauta del profesor](docs/19_checklist_profesor.md)

## URL de la página funcional

**https://yutre3.github.io/diego/**

La URL abre el prototipo navegable del hotel. La interfaz funcional mantiene la misma línea visual del mockup: barra superior, menú lateral, módulos de Habitaciones, Huéspedes, Reservas, Check-in, Check-out, Usuarios e Informes.

## Credenciales de demostración

Administrador: `admin / admin123`

Encargado: `encargado / hotel123`

## Archivos de la página

- [Acceso](prototipo-web/login.html)
- [Inicio](prototipo-web/index.html)
- [Evaluación 1](prototipo-web/evaluacion1.html)
- [Evaluación 2](prototipo-web/evaluacion2.html)
- [Habitaciones](prototipo-web/habitaciones.html)
- [Huéspedes](prototipo-web/huespedes.html)
- [Reservas](prototipo-web/reservas.html)
- [Check-in](prototipo-web/checkin.html)
- [Check-out](prototipo-web/checkout.html)
- [Usuarios](prototipo-web/usuarios.html)
- [Informes](prototipo-web/informes.html)
- [Material del profesor](prototipo-web/ayuda.html)
- [CSS](prototipo-web/styles.css)
- [JavaScript](prototipo-web/app.js)

## Contenido pedido por el profesor

### Evaluación 1

- [Informe de requerimientos IEEE 830 adaptado](docs/01_requerimientos.md)
- [Reglas de negocio](docs/02_reglas_negocio.md)
- [Matriz de trazabilidad](docs/07_trazabilidad.md)

### Evaluación 2

- [Diagrama de casos de uso](entrega-final/02-casos-de-uso.svg)
- [Diagrama de flujo](entrega-final/03-diagrama-flujo.svg)
- [Modelo de base de datos](entrega-final/04-modelo-base-datos.svg)
- [Normalización](entrega-final/05-normalizacion.svg)
- [Sketch](entrega-final/07-sketch.svg)
- [Wireframe](entrega-final/08-wireframe.svg)
- [Mockup](entrega-final/09-mockup.svg)
- [Kanban](entrega-final/11-kanban.md)

## Trazabilidad con los archivos enviados por el profesor

[Ver trazabilidad completa](docs/18_trazabilidad_fuentes_profesor.md)

Los datos que el Caso 6 no define de forma exacta se mantienen identificados como pendientes o como datos de demostración.
