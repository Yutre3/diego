...
function message(id,text,ok=true){const e=$(id);if(!e)return;e.textContent=text;e.className='message '+(ok?'ok':'error')}
...
function renderHome(){
  ...
  $('#statDisponibles').textContent=disp;$('#statOcupadas').textContent=ocup;$('#statReservas').textContent=d.reservas.filter(r=>r.estado!=='CANCELADA').length;$('#statHuespedes').textContent=d.huespedes.length;
...
$('#roomForm').addEventListener('submit',e=>{
  e.preventDefault();const d=db(),num=$('#roomNumber').value.trim();
  if(d.habitaciones.some(r=>r.numero===num))return message('#roomMessage','Ese número ya existe.',false);
  d.habitaciones.push({id:uid(d.habitaciones),numero:num,capacidad:+$('#roomCapacity').value,orientacion:$('#roomOrientation').value,estado:'DISPONIBLE'});
  save(d);e.target.reset();message('#roomMessage','Habitación guardada.');renderAll()
});
...
$('#guestForm').addEventListener('submit',e=>{
  e.preventDefault();const d=db(),name=$('#guestName').value.trim();
  if(!name)return message('#guestMessage','Ingrese un nombre.',false);
  d.huespedes.push({id:uid(d.huespedes),nombre:name});save(d);e.target.reset();message('#guestMessage','Huésped guardado.');renderAll()
});
...
function availableForDates(entry,exit){
  ...
  return d.habitaciones.filter(room=>{
    ...
    return !d.reservas.some(r=>r.habitacionId===room.id && !['FINALIZADA','CANCELADA'].includes(r.estado) && entry<r.salida && exit>r.entrada)
  })
}
...
$('#availabilityForm').addEventListener('submit',e=>{
  e.preventDefault();const a=$('#searchEntry').value,b=$('#searchExit').value;
  if(!a||!b||b<=a)return message('#availabilityMessage','La fecha de salida debe ser posterior a la entrada.',false);
  renderAvailable(a,b);message('#availabilityMessage','Disponibilidad actualizada.')
});
...
function renderReservations(){
  ...
  $('#reservationsList').innerHTML=d.reservas.length?d.reservas.map(r=>{
    ...
    const cancel=r.estado==='REGISTRADA'?'<button type="button" class="danger" data-cancel-reservation="'+r.id+'">Cancelar</button>':'';
    return '<div class="data-row"><div><strong>#'+r.id+' · '+esc(h?.nombre||'—')+' · Hab. '+esc(room?.numero||'—')+'</strong><span class="small">'+esc(r.entrada)+' → '+esc(r.salida)+' · '+r.cantidad+' huésped(es)</span></div><div class="row-actions"><span class="status '+(r.estado==='CANCELADA'?'cancelled':'')+'">'+esc(r.estado)+'</span>'+cancel+'</div></div>'
  }).join(''):'<div class="empty">Aún no hay reservas.</div>';
  $$('[data-cancel-reservation]').forEach(b=>b.addEventListener('click',()=>{
    const d=db(),r=d.reservas.find(x=>x.id===+b.dataset.cancelReservation);
    if(r&&r.estado==='REGISTRADA'){r.estado='CANCELADA';save(d);renderAll();message('#reservationMessage','Reserva cancelada.')}
  }))
}
$('#reservationForm').addEventListener('submit',e=>{
  e.preventDefault();const d=db(),hid=+$('#reservationGuest').value,rid=+$('#reservationRoom').value,entry=$('#reservationEntry').value,exit=$('#reservationExit').value,count=+$('#reservationCount').value,room=d.habitaciones.find(r=>r.id===rid);
...
$('#checkinForm').addEventListener('submit',e=>{
  e.preventDefault();const d=db(),r=d.reservas.find(x=>x.id===+$('#checkinReservation').value);
  if(!r)return message('#checkinMessage','Seleccione una reserva.',false);
  const room=d.habitaciones.find(x=>x.id===r.habitacionId);if(!room||room.estado!=='DISPONIBLE')return message('#checkinMessage','La habitación no está disponible.',false);
  room.estado='OCUPADA';r.estado='CHECKIN';d.estadias.push({id:uid(d.estadias),reservaId:r.id,huespedId:r.huespedId,habitacionId:r.habitacionId,cantidad:r.cantidad,checkin:new Date().toISOString(),checkout:null,total:null,estado:'ACTIVA'});
  save(d);message('#checkinMessage','Check-in registrado. Habitación ocupada.');renderAll()
});
...
$('#checkoutForm').addEventListener('submit',e=>{
  e.preventDefault();const d=db(),s=d.estadias.find(x=>x.id===+$('#checkoutStay').value),cost=+$('#costPerGuest').value||0;
  if(!s)return message('#checkoutMessage','Seleccione una estadía.',false);
  s.total=s.cantidad*cost;s.checkout=new Date().toISOString();s.estado='FINALIZADA';
  const room=d.habitaciones.find(x=>x.id===s.habitacionId);if(room)room.estado='DISPONIBLE';
  const r=d.reservas.find(x=>x.id===s.reservaId);if(r)r.estado='FINALIZADA';
  save(d);message('#checkoutMessage','Check-out registrado. Habitación liberada.');renderAll()
});

function renderUsers(){
  const d=db();$('#usersList').innerHTML=d.usuarios.map(u=>'<div class="data-row"><div><strong>'+esc(u.nombre)+'</strong><span class="small">'+esc(u.rol)+'</span></div></div>').join('')
}
$('#userForm').addEventListener('submit',e=>{
  e.preventDefault();const d=db(),name=$('#userName').value.trim();if(!name)return message('#userMessage','Ingrese un nombre.',false);
  d.usuarios.push({id:uid(d.usuarios),nombre:name,rol:$('#userRole').value});save(d);e.target.reset();message('#userMessage','Usuario guardado.');renderAll()
});

function renderReports(){
  const d=db(),occupied=d.habitaciones.filter(x=>x.estado==='OCUPADA').length,available=d.habitaciones.filter(x=>x.estado==='DISPONIBLE').length,total=d.habitaciones.length;
  $('#reportOccupied').textContent=occupied;$('#reportAvailable').textContent=available;$('#reportReservations').textContent=d.reservas.filter(r=>r.estado!=='CANCELADA').length;$('#reportOccupancy').textContent=total?Math.round(occupied/total*100)+'%':'0%';
  $('#reportList').innerHTML=d.reservas.length?d.reservas.map(r=>{const room=d.habitaciones.find(x=>x.id===r.habitacionId);return '<div class="data-row"><div><strong>Reserva #'+r.id+' · Hab. '+esc(room?.numero||'—')+'</strong><span class="small">'+esc(r.entrada)+' → '+esc(r.salida)+'</span></div><span class="status '+(r.estado==='CANCELADA'?'cancelled':'')+'">'+esc(r.estado)+'</span></div>'}).join(''):'<div class="empty">Sin reservas.</div>'
}

function renderAll(){seed();renderHome();renderRooms();renderGuests();renderReservations();renderAvailable();renderCheckin();renderCheckout();renderUsers();renderReports()}
renderAll();
